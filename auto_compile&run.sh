...
# Aktuellen Verzeichnisnamen ermitteln, falls $directory = "."
if [ "$directory" = "." ]; then
  directory=$(pwd)
fi

# Executablename
executableName=$(basename "$1" .c)
...
if [ $? -eq 0 ]; then
  echo "Kompilierung erfolgreich! Das ausführbare Programm befindet sich im Verzeichnis: $directory mit dem Namen: $executableName"
  $directory/$executableName
else
